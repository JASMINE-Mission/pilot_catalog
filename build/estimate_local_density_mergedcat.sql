...
FROM (SELECT * FROM merged_sources WHERE phot_hw_mag<12) as m,
  LATERAL (
    SELECT q3c_dist(m.ra,m.dec,m2.ra,m2.dec)*3600. as radius,m2.phot_hw_mag as hw FROM (SELECT source_id,ra,dec,phot_hw_mag FROM merged_sources as m1 WHERE q3c_join(m.ra,m.dec,m1.ra,m1.dec,:radius_threshold/3600.)) as m2 WHERE  m2.source_id!=m.source_id and m2.phot_hw_mag<=m.phot_hw_mag+:hwmagdiff_threshold
    ORDER BY radius ASC
    LIMIT 100
  ) neighbours
