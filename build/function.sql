...
CREATE OR REPLACE FUNCTION compute_glon(
  ...
  FLOAT) -- DEC (deg)
RETURNS FLOAT AS $$
  SELECT wrap(DEGREES(ATAN2((+0.4941094278755837*COS(RADIANS($1))-0.4448296299600112*SIN(RADIANS($1)))*COS($2)+0.7469822444972189*SIN($2),(-0.0548755604162154*COS(RADIANS($1))-0.8734370902348850*SIN(RADIANS($1)))*COS($2)-0.4838350155487132*SIN($2))))
$$ LANGUAGE SQL;
