...
MIN(pmra) as pmra,MIN(pmra_error) as pmra_error,MIN(pmdec) as pmdec,MIN(pmdec_error) as pmdec_error,MIN(uwe) as uwe,
 ...
 FROM (SELECT v2.*,CAST(v.source_id AS varchar) as vvv_sid,v.Cl,v.Var FROM vvv42_sources_full as v INNER JOIN virac2_ks16_clean as v2 ON q3c_join(v.ra,v.dec,v2.ra,v2.dec,.5/3600.) ) as aux GROUP BY source_id;
