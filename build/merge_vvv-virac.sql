...
FROM vvv42_sources_full_clean WHERE phot_ks_mag>16 AND (source_id not in (SELECT vvv_source_id1 FROM vvv_virac_common) OR source_id not in (SELECT vvv_source_id2 FROM vvv_virac_common))  LIMIT 10
UNION
SELECT source_id,glon,glat,ra,dec,phot_z_mag,phot_z_mag_error,phot_y_mag,phot_y_mag_error,phot_j_mag,phot_j_mag_error,phot_h_mag,phot_h_mag_error,phot_ks_mag,phot_ks_mag_error,parallax,parallax_error,pmra,pmra_error,pmdec,pmdec_error,uwe,astfit_epochs,asfit_params,ref_epoch,NULL as  Cl,NULL as Var,'VIR' as source
FROM virac2_ks16_clean WHERE source_id not in (SELECT source_id FROM vvv_virac_common)  LIMIT 10;
