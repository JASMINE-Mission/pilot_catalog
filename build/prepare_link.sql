...
SELECT t.source_id, COUNT(neighbours.source_id_neighbour) N, MAX(radius) R, COUNT(neighbours.source_id_neighbour)/(4*PI()*POWER(sin(MAX(radius)/3600*PI()/180/2),2))/(4.25*POWER(10,10)) AS density
FROM tmass_sources_clean as t,
  LATERAL (
    SELECT t2.source_id as source_id_neighbour, q3c_dist(t.ra,t.dec,t2.ra,t2.dec)*3600. as radius
    FROM tmass_sources_clean as t2
    ORDER BY radius ASC
    LIMIT 100
  ) neighbours
