...
SELECT t.source_id,t.glon,t.glat, COUNT(radius) N, MAX(radius) R, COUNT(radius)/(4*PI()*POWER(sin(MAX(radius)/3600*PI()/180/2),2))/(4.25*POWER(10,10)) AS density
...
SELECT v.source_id,v.glon,v.glat, COUNT(radius) N, MAX(radius) R, COUNT(radius)/(4*PI()*POWER(sin(MAX(radius)/3600*PI()/180/2),2))/(4.25*POWER(10,10)) AS density
...
DROP TABLE IF EXISTS vvv4_sources_local_density CASCADE;
CREATE TABLE vvv4_sources_local_density AS
SELECT v.source_id,v.glon,v.glat, COUNT(radius) N, MAX(radius) R, COUNT(radius)/(4*PI()*POWER(sin(MAX(radius)/3600*PI()/180/2),2))/(4.25*POWER(10,10)) AS density
FROM vvv4_sources_clean as v,
  LATERAL (
    SELECT q3c_dist(v.ra,v.dec,v2.ra,v2.dec)*3600. as radius FROM vvv4_sources_clean as v2 WHERE q3c_join(v.ra,v.dec,v2.ra,v2.dec,:radius_threshold_vvv/3600.)
    ORDER BY radius ASC
    LIMIT 100
  ) neighbours
GROUP BY v.source_id;
...
SELECT s.source_id,s.glon,s.glat, COUNT(radius) N, MAX(radius) R, COUNT(radius)/(4*PI()*POWER(sin(MAX(radius)/3600*PI()/180/2),2))/(4.25*POWER(10,10)) AS density
